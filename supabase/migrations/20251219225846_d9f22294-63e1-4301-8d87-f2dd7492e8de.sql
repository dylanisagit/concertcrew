...
CREATE TABLE public.concerts (
  ...
  date DATE NOT NULL,
  venue TEXT NOT NULL,
  ticket_status TEXT DEFAULT 'thinking',
  ticket_url TEXT,
  spotify_url TEXT,
  image_url TEXT,
  review TEXT,
...
-- Create concert_interests table for tracking who's interested
CREATE TABLE public.concert_interests (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  concert_id UUID NOT NULL REFERENCES public.concerts(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  interest_level TEXT NOT NULL DEFAULT 'interested', -- interested, going, maybe
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  UNIQUE (concert_id, user_id)
);

-- Create comments table for concert discussions
...
ALTER PUBLICATION supabase_realtime ADD TABLE public.concert_interests;